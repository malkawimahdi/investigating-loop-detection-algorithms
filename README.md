...
cbmc-bugs contains folders for each generated bug report containing the supporting evidence for each bug report. A text file is contained with the URL to the submitted bug report for each bug.
```
 ┣ 📂cbmc-bugs
...
control-flow-graph-test-cases contains a c program and it's equivalent represenation as a control flow graph. *.goto is an intemediary that is generated by CBMC to generate *.goto. Files named "graphviz.svg" are the raw output control flow graphs. For the other reprsentation of the control flow graphs, CBMC's processing has been truncatted to reference the actual test case.
